...
//수락여부 +소켓
router.patch("/:notificationId/:receiverId", auth, async (req, res) => {
  try{
  let {notificationId,receiverId} = req.params
  const {type} = req.body; //수락하면 type3 // 거절하면 type 4

  // let senderId = res.locals.user.userId;
  const changeReceiverId = receiverId;
  const changeSenderId = res.locals.user.userId;

  const senderId = changeReceiverId;
  receiverId = changeSenderId;
  ...
  const query = `UPDATE notification SET ? WHERE notificationId = '${notificationId}' and notification.senderId = '${userId}'`;
